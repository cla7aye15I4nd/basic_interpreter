...
namespace basic{
    ...
    class basic_interpreter{
        ...
        int eval(const expr_iter& begin,
            ...
            int result = *atom.begin();
            for (size_t p = 0; p < oper.size(); ++p) {
    ...
    public:
        ...
        bool check_expr(const expr_iter& begin,
            ...
            for (auto ptr = begin; ptr != end; ptr++) {
                ...
                if (*ptr == ")") if (--cnt < 0) return false;
                if (is_element(*ptr) && ptr + 1 != end && *(ptr + 1) != ")") {
                    if (ptr + 2 == end || (!is_element(*(ptr + 2)) && *(ptr + 2) != "("))
                        return false;
                }
            }
...
#define REPORT(var) if (var) return false;
        ...
        bool feedback(const expression& expr) {
            step s;
            s.expr = expr;
